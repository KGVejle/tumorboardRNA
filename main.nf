...
def helpMessage() {
    ...
    Main options:
      ...
      --deletework            delete the workfolder generated by the nextflow script.
                                Default: not set - does not removes the Work folder
...
workflow.onComplete {
    ...
    if (!params.nomail && workflow.duration > 1200000 && workflow.success) {
        if (System.getenv("USER") in ["raspau", "mmaj"]) {
            ...
            ---------------------------
            Pipeline completed  : Tumorboard RNA ${samplesheetDate}
    ...
    if (params.deletework && workflow.duration > 1200000 && workflow.success) {
